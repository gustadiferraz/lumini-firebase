...
function getStatusInfo(employee) {
  ...
  if (employee.status === 'promoted') return { label: '⭐ Promovido', cls: 'status-promoted', pct };
  if (employee.status === 'approved') return { label: '✅ Aprovado', cls: 'status-approved', pct };
  if (months >= Number(employee.minMonths || 0)) return { label: '🟡 Apto', cls: 'status-ready', pct: 100 };
  return { label: '🔴 Em período', cls: 'status-period', pct };
}
...
function renderAdminDashboard() {
  ...
  const statusCount = {
    ...
    ready: employees.filter(emp => getStatusInfo(emp).cls === 'status-ready').length,
    approved: employees.filter(emp => emp.status === 'approved').length,
    promoted: employees.filter(emp => emp.status === 'promoted').length
  };
  ...
  chartStatus = new Chart(document.getElementById('chart-status'), {
    type: 'bar',
    data: {
      labels: ['Em período', 'Apto', 'Aprovado', 'Promovido'],
      datasets: [{
        data: [statusCount.period, statusCount.ready, statusCount.approved, statusCount.promoted],
        backgroundColor: ['#FEE2E2', '#FEF3C7', '#DCFCE7', '#DBEAFE'],
  ...
  chartPayroll = new Chart(document.getElementById('chart-payroll'), {
    type: 'doughnut',
    data: {
      labels: ['Conferidas', 'Pendentes'],
      datasets: [{
        data: [verified, pending.length],
